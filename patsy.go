...
//go:generate becca -package=github.com/AviSantoso/patsy
...
func Dirs(env vos.Env, packagePath string) (map[string]string, error) {
	...
	exe := exec.Command("go", "list", "-f", "{{.ImportPath}}___{{.Dir}}", packagePath)
	exe.Dir = wd
	exe.Env = env.Environ()
	...
	for _, line := range lines {
		...
		chunks := strings.Split(strings.TrimSpace(line), "___")
		importPath := chunks[0]
		dir := chunks[1]
